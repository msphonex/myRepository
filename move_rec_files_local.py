...
PATH_SRC = r"F:\code\monitor"
PATH_DES = r"G:\rec_files_after_2022"
PATH_LOG = r"F:\code\move_rec_file_local.log"
...
def move_file(des_path, src_path, log_path):
    ...
    log_file.flush()
    move_time = (now - datetime.timedelta(days=1)).strftime('%Y%m%d')
    ...
    for file_name in record_files:  # re match two days ago rec files then move them
        ...
        if move_files:
            if os.path.exists("{0}\{1}".format(src_path, file_name)):

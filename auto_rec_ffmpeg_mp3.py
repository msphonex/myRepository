...
def auto_rec_ffmpeg_audio(url, rec_path, log_path):
    ...
    ff = ffmpy3.FFmpeg(
        inputs={url: '-t 04:00:00 -rtsp_transport tcp'},
        outputs={rec_path: '-vn -map 0:1 -c:a:1 acc -strict experimental -y'}
    )
...
def auto_rec_ffmpeg_video(url, rec_path, log_path):
    ...
    ff = ffmpy3.FFmpeg(
        inputs={url: '-rtsp_transport tcp'},
        outputs={rec_path: '-t 04:00:00 -an -map 0:0 -c:v:0 libx265 -g 60 -r 15 -y'}
    )
    log_file.write(ff.cmd)
    log_file.write("\n")
    ff.run(stdout=log_file, stderr=log_file)
...
if __name__ == '__main__':
    while True:
        ...
        path_audio = r"{0}\{1}_pre.wav".format(PATH_REC_FILES, format_str_time)
